...
window.AiceSlide3 = (function () {
  ...
  /* ----- 내용을 자산(1·2단계) 상태로 되돌림 ----- */
  function setVal(el, txt) {
    if (el) { el.innerHTML = txt; gsap.set(el, { yPercent: 0, opacity: 1 }); }
  }
  function resetContent() {
    setVal(document.getElementById('s3-v-when'),    ASSET.when);
    setVal(document.getElementById('s3-v-where'),   ASSET.where);
    setVal(document.getElementById('s3-v-howmuch'), ASSET.howmuch);
    setVal(document.getElementById('s3-tag'),       '가계부');
  }
  ...
  function build() {
    ...
    var field0    = document.getElementById('s3-field-0');
    var field2    = document.getElementById('s3-field-2');
    ...
    var vWhen     = document.getElementById('s3-v-when');
    var vWhere    = document.getElementById('s3-v-where');
    var vHowmuch  = document.getElementById('s3-v-howmuch');
    var vals      = [vWhen, vWhere, vHowmuch];
    var rule      = document.getElementById('s3-rule');
    var steps     = document.getElementById('s3-steps');
    if (!lump || !field0 || labels.length !== 3) return;
    ...
    gsap.set(labels, { filter: WORD_OFF });
    setStep(0);
    ...
    tlExpand.fromTo(field0, { x: 120 }, { x: 0, duration: 0.72, ease: 'expo.out' }, 0.28);
    tlExpand.fromTo(field2, { x: -120 }, { x: 0, duration: 0.72, ease: 'expo.out' }, 0.28);
    ...
    /* ── 에너지 전환 (Stage 2 → 3 — 같은 구조가 에너지로) ── */
    tlEnergy = gsap.timeline({ paused: true, defaults: { ease: 'power3.out' } });
    tlEnergy.add(flipText(tag,      '에너지 가계부'),   0);
    tlEnergy.add(flipText(vWhen,    ENERGY.when),    0.05);
    tlEnergy.add(flipText(vWhere,   ENERGY.where),   0.16);
    tlEnergy.add(flipText(vHowmuch, ENERGY.howmuch), 0.27);
    tlEnergy.to(labels, { scale: 1.1, duration: 0.22, yoyo: true, repeat: 1,
                          stagger: 0.07, ease: 'power2.inOut' }, 0.22);
  }
